    ...
    <div className='py-20 px-6 overflow-hidden mx-auto bg-slate-900/10 dark:bg-slate-100/10 w-full'>
      <div className='max-w-xl mx-auto flex flex-col gap-y-10 items-center w-full'>
        <nav className='grid grid-cols-2 gap-x-2 gap-y-4 w-full lg:text-center'>
          {
            [
              {selector: '#hero', text: 'Anasayfa'},
              {selector: '#projects', text: 'Projeler'},
              {selector: '#testimonials', text: 'Referanslar'},
              {selector: '#stats', text: 'İstatistikler'},
              {selector: '#feature', text: 'Hizmetler'},
            ].map(btn => (
              <button onClick={() => {
                window.scrollTo({
                  top: (document.querySelector(btn.selector)?.getBoundingClientRect().top || 0) + window.pageYOffset - 80,
                  behavior: 'smooth',
                })
              }} className='px-2 py-1 rounded-lg hover:bg-slate-900/10 transition-colors'>{btn.text}</button>
            ))
          }
        </nav>
